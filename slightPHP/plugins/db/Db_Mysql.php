...
class Db_Mysql extends DbObject{
	 ...
	 */
	function select($table,$condition="",$item="*",$groupby="",$orderby="",$leftjoin="", $having=""){
		if($item==""){$item="*";}
		if(is_array($table)){
		...
		if(!is_array($having)&&$having!="") {
			...
			$this->sql="SELECT $item FROM $table $join $condiStr $groupby $having $orderby_sql $limit";
			$this->countsql="SELECT count(1) totalSize FROM (SELECT $item FROM $table $join $condiStr $groupby $having) a";
		}else{
			$this->sql="SELECT $item FROM $table $join $condiStr $groupby $orderby_sql $limit";
			$this->countsql= "SELECT count(1) totalSize FROM (SELECT $item FROM $table $join $condiStr $groupby) a";
		}

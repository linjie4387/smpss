...
class c_delivery extends base_c {
    ...
    function pagedeliverygoodsbyorder($inPath) {
        ...
        $sql = "select b.* from smpss_delivery a ,smpss_deliverywithgoods b, smpss_order c where a.delivery_id = b.delivery_id and b.order_id = c.order_id and ifnull(if(c.fid=0,c.order_id,c.fid),c.order_id) =".$order_id;
        ...
        //echo '1ddddddd';
		//var_dump($this->params['goodsList']);
        $datadictObj = new m_datadict();
        $this->params['signstatusList'] = $datadictObj->getOrderType(base_Constant::KEY_SIGN_STATUS);
    ...
    function pageeditwithgoods($inPath){
        ...
        if ($rs) {
            $this->ajax_res("操作成功:{$withgoods_id}'--'".$sql, 0);
            exit;
        }
        $this->ajax_res("操作失败：".$deliverywithgoodsObj->getError(), -1);
        exit;
...
本想做成树状，想了下，还是算了
	...
	$url = $this->getUrlParams($inPath);
		...
		//把父订单查出来。
		$sql = "select fid ,order_id,hospital_name,office_name,'商品' AS goods_type,1 AS is_for_goods,delivery_no,delivery_time,large_size_count,remark";
		$sql = $sql." from smpss_ordermeta where (ifnull(fid,0)=0) and sign_status=1 and is_valid = 1 and status = 3".$orderby;
		...
		if($deliveryList) {
			foreach($deliveryList as $key => $val){
				...
				if($val['is_for_goods'] == 1){  //假如是商品（0：发票；1：商品）
					$deliveryList[$key]['20l_quantity_quantity'] = $this->get20lquantity($val['large_size_count'],$val['order_id']);
	...
	function get20lquantity($large_size_count,$orderid){
		if(large_size_count == 0){  //没有大包装
			...
			return $quantityObj['20l_quantity_quantity'];
		}else{ 
			return $large_size_count;
		}
...
/***备份一份代码***/
	...
	function pageaddwithgoods($inPath) {
		...
		$deliverywithgoodsObj = new m_deliverywithgoods();				
		$withgoods = $deliverywithgoodsObj->select("delivery_id = {$delivery_id}","order_id,is_for_goods");
		
		$goodslist = array();
		foreach($withgoods->items as $val){			
			$goodslist[] = 'oid_'.$val['order_id'].'_'.$val['is_for_goods'];
		}
				
		$deliveryObj = new m_delivery();
		
		$sql = "select order_id, hospital_name, office_name, goods_type, is_for_goods, goods_no from (select order_id, hospital_name, office_name, '商品' goods_type,1 is_for_goods, delivery_no goods_no from smpss_order where is_goods_signed = 0 and is_valid = 1 and status = 3 union all select order_id, hospital_name, office_name, '发票' goods_type,0 is_for_goods, invoice_no goods_no from smpss_order where is_invoice_signed = 0 and is_valid = 1 and status = 3) unsigned_order order by order_id, goods_type";
		...
		foreach($deliveryList as $key => $val){
			$deliveryList[$key]['ischeck'] = false;
			$curid = 'oid_'.$val['order_id'].'_'.$val['is_for_goods'];
			
			if(in_array($curid,$goodslist)){
				$deliveryList[$key]['ischeck'] = true;
			}
		}

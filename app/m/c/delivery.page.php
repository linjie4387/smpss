...
class c_delivery extends base_c {
    ...
    function pageindex($inPath) {
        ...
        if ($_POST) {
            $key = base_Utils::getStr($_POST['key'], 'html');
            if ($key) {
                $condition .= "(car_license like '%{$key}%' or driver_name like '%{$key}%' or exists (select 1 from smpss_deliverywithgoods g where g.delivery_id=smpss_delivery.delivery_id and g.order_id={$key}))";
                $this->params['key'] = $key;
            }
		...
		if($deliveryList) {
			foreach($deliveryList as $key => $val){
				...
				$where='select a.*,b.hospital_name,b.office_name,b.delivery_no,b.invoice_no,b.remark,if(a.is_for_goods=0,\'发票\',\'商品\') type,b.is_valid from smpss_deliverywithgoods a,smpss_order b where a.order_id=b.order_id and a.delivery_id='.$val['delivery_id'];
				//echo $where;
				$rs = $deliverywithgoodsObj->query($where);
				$list = $rs->items;
			 	if (!empty($list)) {
					$cnt = count($list);
        ...
        return $this->render('delivery/index1.html', $this->params);
    }
	...
	//edit by linj，用于动态地获取配送任务
    function pagedeliverygoodslistjson($inPath) {
        $url = $this->getUrlParams ( $inPath );
        $page = $url ['page'] ? ( int ) $url ['page'] : 1;
		...
		echo json_encode($this->params['goodsList']);
    }
    ...
    function pageeditwithgoods($inPath){
        $url = $this->getUrlParams($inPath);
        $withgoods_id = (int)$url['wid'] > 0 ? (int)$url ['wid'] : (int)$_POST['withgoods_id'];
        $info = ['sign_status'=>(string)$_POST['isSign'],'remark'=>(string)$_POST['remark']];
        $deliverywithgoodsObj = new m_deliverywithgoods();
	...
	function pageaddhasdelivery($inPath) {
		...
		if($rs){
			...
			$this->ShowMsg("操作失败！", $this->createUrl("/delivery/hasdeliverylist"), 2, 1);
		}
    }

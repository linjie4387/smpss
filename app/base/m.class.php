...
class base_m {
	...
	function __construct($pkid = false) {
		...
		$this->_db = SDb::getDbEngine ( $this->_dbConfig["engine"] );
...
//		return $this->_db->query ( $this->tableName (), $sql, $bind1, $bind2, $params );
        return $this->_db->query ( $sql, $bind1, $bind2, $params );
	}
	 ...
	 * 生成订单号:15位订单号
	 */
	function buildOrderNo($orderType) {
		if (! $orderType) {
			throw new Exception ( "缺少订单类型！" );
		}
		$random = substr ( implode ( NULL, array_map ( 'ord', str_split ( substr ( uniqid ("", true), 10, 13 ), 1 ) ) ), 0, 5 );
		return str_pad ( $orderType . date ( 'Ym' ), 8, "0", STR_PAD_RIGHT ) . $random;
	}
}
